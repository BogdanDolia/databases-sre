...
- `pgAdmin`, `pganalyze`, `Prometheus + Grafana`: collecting and visualizing database metrics (connections, latencies,
  table sizes, indexing, etc.).
...
- External caching (`Redis`, `Memcached`) is sometimes used when very fast responses are required without accessing the
  database.
